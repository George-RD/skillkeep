 ...
 * a file-backed `bun:sqlite` handle (db.test.ts's migration test) and then immediately `rm` their
 * own tmpDir: even after `db.close()` returns, Windows doesn't always release the underlying OS
 * file handle (the db + its -wal/-shm siblings) in the same tick, so a bare `fs.rmSync` can throw
